...
//generate booking
function generateBooking(args) {
    args.reservations = [];
    if (args.itemsPerPage == null || args.itemsPerPage == undefined)
        args.itemsPerPage = 10;
    (function (args) {
        $.ajax({
            url: host + '/api/Booking/AllReservations' + '?' + $.param({
                Page: args.page,
                PageSize: args.itemsPerPage
            }),
            dataType: 'json',
            success: function (data, status, xhr) {
                for (var i = 0; i < data.Reservations.length; i++) {
                    var reservation = convertReservationFromExternReprezentationToIntern(data.Reservations[i]);
                    args.reservations.push(reservation);
                }
                args.totalItems = data.TotalItems;
                if (args.totalItems == 0) {
                    args.container.html("No reservations!");
                    return;
                }
                generateHtmlTable(args);
                activateTableFeaturesReservations(args);
            },
            error: function (xhr, status, error) {
                alert("Fail load hotels! \n" + error);
            }
...
function generatePaginationContainerApi(args) {
    ...
    var nrPages = 0;
    nrPages = parseInt(args.totalItems / args.itemsPerPage) + 1;
...
function generateHtmlTable(args) {
    ...
    if (args.hotels == null || args.hotels == undefined) {
        items = args.reservations;
    } else {
        items = args.hotels.list;
    }
    for (var i = 0 ; i < items.length ; i++) {
        tbody.append(createHtmlRowTable(items[i], args.columns));
    }
    if (args.enableModify) {
        $(divTemp)
            .append('<input id=\'addButton\' type=\'button\' value=\'Add\'/>')
...
function createEditRow(hotel, columns) {
    ...
    if (columns === undefined || columns === null) {
        ...
        for (var i = 0 ; i < columns.length; i++) {
            if (columns[i].type === undefined || columns[i].type === null) {
                if (columns[i].toLowerCase() !== 'operations') {
                    row += '<td><input data-id="' + columns[i].field + '" value=\'' + hotel[columns[i].field] + '\' type="text"  /></td>';
                }
                else {
                    row += '<td>' + createHtmlDivOperations() + '</td>';
                }
                ...
                if (columns[i].field.toLowerCase() !== 'operations') {
                    row += '<td><input data-id="' + columns[i].field + '" value=\'' + hotel[columns[i].field] + '\' type="' + columns[i].type +'" /></td>';
                } else {
                    row += '<td>' + createHtmlDivOperations() + '</td>';
                }
            }
...
//convert reservation from external
function convertReservationFromExternReprezentationToIntern(reservation) {
    var reservation = {
        id: reservation.Id,
        ...
        checkin: reservation.CheckIn.substring(0,10),
        checkout: reservation.CheckOut.substring(0,10)
    };
    return reservation;
}
